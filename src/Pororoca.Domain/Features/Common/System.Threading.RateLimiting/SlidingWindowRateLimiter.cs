...
namespace System.Threading.RateLimiting
{
    ...
    public sealed class SlidingWindowRateLimiter : ReplenishingRateLimiter
    {
        ...
        public SlidingWindowRateLimiter(SlidingWindowRateLimiterOptions options)
        {
            ...
            {
                throw new ArgumentException("ShouldBeGreaterThan0", nameof(options.PermitLimit));
            }
            if (options.SegmentsPerWindow <= 0)
            {
                throw new ArgumentException("ShouldBeGreaterThan0", nameof(options.SegmentsPerWindow));
            }
            if (options.QueueLimit < 0)
            {
                throw new ArgumentException("ShouldBeGreaterThanOrEqual0", nameof(options.QueueLimit));
            }
            if (options.Window <= TimeSpan.Zero)
            {
                throw new ArgumentException("ShouldBeGreaterThanTimeSpan0", nameof(options.Window));
            }
        ...
        protected override RateLimitLease AttemptAcquireCore(int permitCount)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(permitCount), permitCount, "PermitLimitExceeded");
            }
